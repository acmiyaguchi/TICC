...
class TICC:
    ...
    def fit(self, input_file):
        ...
        gmm = gmmx.GaussianMixtureModelJax.create(
            n_components=self.number_of_clusters, n_features=d
        )
        ...
        start = time.time()
        kmeans = faiss.Kmeans(
            d, self.number_of_clusters, niter=20, verbose=False, gpu=False
        )
        ...
        pool = ctx.Pool(processes=self.num_proc)  # multi-threading
    ...
    def write_plot(self, clustered_points, str_NULL, training_indices):
        ...
        plt.figure()
        plt.plot(
            training_indices[0 : len(clustered_points)], clustered_points, color="r"
        )  # ,marker = ".",s =100)
